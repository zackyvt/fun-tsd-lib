...
export const pr = (str: string, ...rest:string[]):void=>{ // prints no trailing newline
    filePrintTrailer(Deno.stdout, "", str, ...rest);
};
...
export const getWorkingDirectory = Deno.cwd;
...
export const newFileReader = (fileName: string): JStyleIter<string> => {
  return new JStyleIter(
    lines(Deno.openSync(fileName, { read: true, write: true }))
  );
};
...
export const newFileWriter = (fileName: string): Deno.File => {
  return Deno.createSync(fileName);
};

// var newFileAppender = function(fileName){return new java.io.FileWriter(fileName, true);}
export const newFileAppender = (fileName: string): Deno.File => {
  return Deno.openSync(fileName, { write: true, append: true, create: true });
};

export const filePrint = (
  openedFileWriterAppender: Deno.File,
  str: string,
  ...rest:string[]
) => {
...
export const fileClose = (openedFileWriterAppender: Deno.File): void => {
