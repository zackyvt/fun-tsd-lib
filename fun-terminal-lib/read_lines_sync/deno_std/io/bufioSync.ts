// Based on https://raw.githubusercontent.com/denoland/deno/master/std/io/bufio.ts
// Based on https://github.com/golang/go/blob/891682/src/bufio/bufio.go
// Copyright 2009 The Go Authors. All rights reserved.
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.

type Reader = Deno.SyncReader;
type Writer = Deno.SyncWriter;
import { charCode, copyBytes } from "https://deno.land/std@v0.35.0/io/util.ts";
...
export class BufReader implements Reader {
   ...
   */
  readFull(p: Uint8Array): Uint8Array | Deno.EOF {
    let bytesRead = 0;
   ...
   */
  readString(delim: string): string | Deno.EOF {
    if (delim.length !== 1)
   ...
   */
  readLine(): ReadLineResult | Deno.EOF {
   ...
   */
  readSlice(delim: number): Uint8Array | Deno.EOF {
   ...
   */
  peek(n: number): Uint8Array | Deno.EOF {
    if (n < 0) {
...
export class BufWriter implements Writer {
  ...
  flush(): void {
    ...
    let n = 0;
    try {
      n = this.wr.writeSync(this.buf.subarray(0, this.n));
    } catch (e) {
      this.err = e;
      throw e;
    }
...
export function* readStringDelim(
  reader: Reader,
  delim: string
): IterableIterator<string> {
  const encoder = new TextEncoder();
  const decoder = new TextDecoder();
  for (const chunk of readDelim(reader, encoder.encode(delim))) {
